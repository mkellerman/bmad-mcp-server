 ...
 */
export function detectBmadSource(sourcePath) {
 ...
 */
function detectV6Structure(sourcePath) {
    ...
    if (fs.existsSync(manifestPath)) {
        try {
            ...
            manifestData = yaml.load(content);
        }
        catch (error) {
 ...
 */
function detectV4Structure(sourcePath) {
    ...
    if ((base === '.bmad' || base === '.bmad-core') &&
        fs.statSync(sourcePath).isDirectory()) {
        coreDir = sourcePath;
    }
    else if (fs.existsSync(path.join(sourcePath, '.bmad')) &&
        fs.statSync(path.join(sourcePath, '.bmad')).isDirectory()) {
        coreDir = path.join(sourcePath, '.bmad');
    }
    else if (fs.existsSync(path.join(sourcePath, '.bmad-core')) &&
        ...
        const hasInstall = fs.existsSync(path.join(sourcePath, 'install-manifest.yaml'));
        const hasCoreConfig = fs.existsSync(path.join(sourcePath, 'core-config.yaml'));
    ...
    if (!coreDir) {
        return {
            ...
            type: 'unknown',
            path: sourcePath,
            error: 'No v4 markers found (.bmad/, .bmad-core/, core-config.yaml, or install-manifest.yaml)',
        };
    }
    ...
    if (fs.existsSync(manifestPath)) {
        try {
            ...
            return {
                ...
                type: 'unknown',
                path: coreDir,
                error: `Failed to parse install-manifest.yaml: ${error instanceof Error ? error.message : String(error)}`,
            };
        }
    }
    ...
    const version = coreConfig?.version ?? manifestData?.version;
    if (!version) {
        return {
            isValid: false,
            type: 'unknown',
    ...
    let expansionPacks;
    if (manifestData?.expansion_packs !== undefined &&
...
//# sourceMappingURL=bmad-source-detector.js.map