 ...
 */
export function parseQualifiedName(input) {
    const trimmed = input.trim();
    // Check for module/name pattern (contains forward slash)
    if (trimmed.includes('/')) {
        const slashIndex = trimmed.indexOf('/');
        const module = trimmed.substring(0, slashIndex);
        const name = trimmed.substring(slashIndex + 1);
        // Validate we have both parts
        if (module && name) {
            logger.info(`Parsed qualified name: module="${module}", name="${name}"`);
            return {
                module,
                name,
                original: trimmed,
        ...
        // Invalid format (e.g., "/name" or "module/")
        logger.warn(`Invalid qualified name format: "${trimmed}"`);
    }
    // Simple name (no module qualifier)
    logger.info(`Parsed simple name: "${trimmed}"`);
    return {
        name: trimmed,
        original: trimmed,
    };
 ...
 */
export function findRecordByName(records, parsed) {
    // Step 1: Apply module filter if specified
    let candidates = records;
    if (parsed.module) {
        candidates = records.filter((r) => r.moduleName === parsed.module);
        logger.info(`Module filter "${parsed.module}": ${candidates.length}/${records.length} candidates`);
    }
    // Step 2: Find by name
    const match = candidates.find((r) => r.name === parsed.name);
    if (match) {
        logger.info(`Found match for "${parsed.original}": ` +
            `name="${match.name}", module="${match.moduleName}"`);
    }
    else {
        logger.warn(`No match found for "${parsed.original}"`);
    }
    return match;
}
/**
 ...
 */
export function hasModuleQualifier(parsed) {
    return parsed.module !== undefined;
}
/**
 ...
 */
export function formatParsedName(parsed) {
    if (parsed.module) {
        return `${parsed.module}/${parsed.name}`;
    }
    return parsed.name;
}
