...
class AgentCache {
    ...
    maxSize = 100; // Max 100 cached agents
    set(ref, content) {
        // Evict oldest if at capacity
        if (this.cache.size >= this.maxSize) {
            const oldest = Array.from(this.cache.entries()).sort((a, b) => a[1].loadedAt - b[1].loadedAt)[0];
            if (oldest) {
 ...
 */
export function parseRemoteAgentRef(input) {
    // Must start with @
    if (!input.startsWith('@')) {
        return null;
    }
 ...
 */
export function scanModuleDirectory(modulePath, moduleName, sourceLocation) {
    const agents = [];
    const workflows = [];
    const tasks = [];
    const remoteName = sourceLocation.split(':')[0].replace('@', '');
    const origin = {
        kind: 'package', // Remote modules treated as package-level
 ...
 * @internal
 */
function resolveRemoteUrl(remote, registry) {
 ...
 * @internal
 */
async function cloneRemoteRepository(remote, remoteUrl) {
    try {
 ...
 */
export async function loadRemoteModule(ref, registry) {
    // Check cache first
    ...
    contentParts.push('');
    contentParts.push(`✅ Module resources have been registered and are now available for use.`);
    contentParts.push('');
    if (scanned.agents.length > 0) {
 ...
 */
export async function loadRemoteAgent(ref, registry) {
    // Check cache first
    const cached = agentCache.get(ref.fullRef);
    if (cached) {
        return cached;
...
//# sourceMappingURL=dynamic-agent-loader.js.map