...
function initHelp() {
    ...
    return { success: true, type: 'help', content: lines.join('\n'), exitCode: 0 };
}
...
// Legacy initialization helpers (currently disabled)
function _performInitializationLegacy(options, ctx) {
    const target = resolveInitTarget(options, ctx);
    if (!target) {
        return { success: false, exitCode: 1, error: 'Initialization aborted: unable to determine target directory.' };
    }
    ...
    if (fs.existsSync(target)) {
        ...
        if (!stats.isDirectory()) {
            return { success: false, exitCode: 1, error: `Target path exists and is not a directory: ${target}` };
        }
        const contents = fs.readdirSync(target);
        if (contents.length > 0) {
            return { success: false, exitCode: 1, error: `Target directory is not empty: ${target}. Choose an empty directory or remove existing files.` };
        }
    }
    else {
    ...
    try {
        ...
        return { success: false, exitCode: 1, error: `Failed to copy BMAD templates: ${errorMessage}` };
    }
}
