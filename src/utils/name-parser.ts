 ...
 */
export function parseQualifiedName(input: string): ParsedName {
 ...
 */
export function findRecordByName<
  T extends { name?: string; moduleName: string },
>(records: T[], parsed: ParsedName): T | undefined {
  ...
  if (parsed.module) {
    ...
    logger.info(
      `Module filter "${parsed.module}": ${candidates.length}/${records.length} candidates`,
    );
  }
  ...
  if (match) {
    logger.info(
      `Found match for "${parsed.original}": ` +
 ...
 */
export function hasModuleQualifier(parsed: ParsedName): boolean {
