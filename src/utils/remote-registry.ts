 ...
 */
const BUILTIN_REMOTES = new Map<string, string>([
 ...
 */
export function parseRemoteArgs(args: string[]): RemoteRegistry {
  ...
  for (const arg of args) {
    ...
    if (commaIndex === -1) {
      console.warn(
        `[remote-registry] Invalid --remote format (missing comma): ${arg}`,
      );
      continue;
    }
    ...
    if (!isValidRemoteName(name)) {
      console.warn(
        `[remote-registry] Invalid remote name '${name}': must start with letter, contain only lowercase alphanumeric and hyphens`,
      );
      continue;
    }
    ...
    if (!isValidGitUrl(url)) {
      console.warn(
        `[remote-registry] Invalid remote URL for '${name}': must start with git+https://`,
      );
      continue;
    }
 ...
 */
export function parseRemotePath(input: string): {
  remote: string;
  path: string;
} {
  if (!input.startsWith('@')) {
  ...
  if (colonIndex === -1) {
    throw new Error(
      `Invalid remote path format. Use: @remote:path or @remote:/path`,
    );
  }
...
export function resolveRemotePath(
  ...
  const pathSeparatorIndex = baseUrl.indexOf(
    ':/',
 ...
 */
export function listRemotes(registry: RemoteRegistry): Array<{
  name: string;
  url: string;
  isBuiltin: boolean;
}> {
  const result: Array<{ name: string; url: string; isBuiltin: boolean }> = [];
