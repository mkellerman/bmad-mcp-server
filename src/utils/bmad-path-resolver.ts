...
export type BmadLocationSource =
	| 'project'
	| 'cli'
	| 'env'
	| 'user'
 ...
 */
export function resolveBmadPaths(options: ResolveBmadPathsOptions): BmadPathResolution {
	...
	const packageBmadPath = packageBmadCandidates.find(candidate => fs.existsSync(candidate)) ?? packageBmadCandidates[0];
 ...
 */
function findManifestDirectory(candidateRoot: string): ManifestInfo | undefined {
...
export function detectManifestDirectory(candidatePath: string): ManifestInfo | undefined {
