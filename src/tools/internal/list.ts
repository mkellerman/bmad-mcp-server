...
export async function handleListCommand(
  ...
  if (cmd === '*list-agents') {
    ...
    const parsedAgents = allAgentRecords.map((record: MasterRecord) =>
  ...
  if (cmd === '*list-workflows') {
    ...
    const seenWorkflows = new Set<string>(); // Track unique workflow names
    ...
    for (const r of resolved.workflows.filter((x: unknown) => {
      ...
      const workflowName =
        workflow.name ?? workflow.moduleRelativePath ?? 'unknown';

      // Skip duplicate workflows (same name already seen)
      if (seenWorkflows.has(workflowName)) continue;
    ...
    const sortedWorkflows = allWorkflows.sort((a: any, b: any) => {
    ...
    markdown.push('---\n');
    markdown.push(
      '**Tip:** Execute any workflow with `bmad *workflow-name`, e.g., `bmad *party-mode` or `bmad *dev-story`',
    );
  ...
  if (cmd.startsWith('*list-agents @')) {
    ...
    const installedAgents = new Set<string>(
      master.agents.map((a: MasterRecord) => a.name).filter(Boolean),
    );
  ...
  if (cmd.startsWith('*list-modules @')) {
    ...
    const installedModules = new Set<string>(
      discovery.locations
        .filter((loc: any) => loc.status === 'valid')
        .map((loc: any) => loc.moduleName)
        .filter(Boolean),
    );
