...
export function handleListCommand(
  ...
  if (cmd === '*list-workflows') {
    ...
    for (const r of resolved.workflows.filter(
      (x: any) => x.kind === 'workflow',
    )) {
  ...
  if (cmd === '*list-modules') {
    ...
    const byModule = new Map<
      string,
      { agents: number; workflows: number; tasks: number }
    >();
    ...
    for (const [mod, counts] of mods) {
      lines.push(
        `| ${mod} @ ${origin} | ${counts.agents} | ${counts.workflows} | ${counts.tasks} |`,
      );
    }
    return {
      success: true,
