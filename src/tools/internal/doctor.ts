...
export function doctor(command: string, ctx: DoctorContext): BMADToolResult {
  ...
  const activeLocation = localDiscovery.locations.find(
  ...
  if (activeLocation) {
    ...
    lines.push(
      `✅ **Active Location:** ${relPath} (${activeLocation.displayName})`,
    );
    lines.push(`- **Version:** ${versionStr}`);
    lines.push(
      `- **Format:** ${active.version === 'v6' ? 'Modern modular structure' : active.version === 'v4' ? 'Legacy dotfolder structure' : 'Custom installation'}`,
    );
  ...
  for (const location of localDiscovery.locations) {
    ...
    lines.push(
      `${statusIcon} **${location.displayName}:** \`${shortPath}\`${detail}`,
    );
  }
  ...
  if (master) {
    ...
    lines.push(
      `| Agents    | ${verifiedAgents} | ${notInManifestAgents} | ${noFileFoundAgents} | ${agentStatus} |`,
    );
    lines.push(
      `| Workflows | ${verifiedWorkflows} | ${notInManifestWorkflows} | ${noFileFoundWorkflows} | ${workflowStatus} |`,
    );
    lines.push(
      `| Tasks     | ${verifiedTasks} | ${notInManifestTasks} | ${noFileFoundTasks} | ${taskStatus} |\n`,
    );
    ...
    if (notInManifest.length || noFileFound.length) {
      ...
      if (notInManifest.length) {
        lines.push(
          `**Untracked Files:** ${notInManifest.length} files found on disk but not in manifest\n`,
        );
      ...
      if (noFileFound.length) {
        lines.push(
          `**Missing Files:** ${noFileFound.length} files listed in manifest but not found on disk\n`,
        );
