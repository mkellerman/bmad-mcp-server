...
import { resolveBmadPaths, type BmadPathResolution } from '../../utils/bmad-path-resolver.js';
...
export function doctor(command: string, ctx: DoctorContext): BMADToolResult {
  ...
  const master: MasterManifests | null = service ? (reload ? service.reload() : service.get()) : null;
  ...
  if (master) {
    ...
    const fmt = (verified: number, notInManifest: number, noFileFound: number) =>
    ...
    // Issues Snapshot
    const topN = 8;
    const notInManifest = [...master.agents, ...master.workflows, ...master.tasks].filter((r) => r.status === 'not-in-manifest');
    const noFileFound = [...master.agents, ...master.workflows, ...master.tasks].filter((r) => r.status === 'no-file-found');
    if (notInManifest.length || noFileFound.length) {
      ...
      if (notInManifest.length) {
        ...
        if (notInManifest.length > topN) lines.push(`│    • ...and ${notInManifest.length - topN} more`);
      }
      if (noFileFound.length) {
        ...
        if (noFileFound.length > topN) lines.push(`│    • ...and ${noFileFound.length - topN} more`);
      }
      lines.push('└────────────────────────────────────────────────────────────┘');
      lines.push('');
    }
