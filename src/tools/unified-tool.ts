...
export class UnifiedBMADTool {
  ...
  private discover(fullReport = false): BMADToolResult {
    ...
    if (!fullReport) {
      ...
      lines.push('');
      lines.push(
        `**Active Location:** ${active.displayName} (Priority ${active.priority})`,
      );
      ...
      for (const location of sorted) {
        ...
        lines.push(
          `${location.priority}. ${location.displayName}${marker} - \`${this.formatLocationPath(location)}\``,
        );
      }
      lines.push('');
      ...
      lines.push('');
      lines.push(
        '*For detailed file-by-file listing, run:* `bmad *doctor --full`',
      );
    } else {
      ...
      lines.push('');
      lines.push(
        '- ✅ **Registered**: File exists and is in manifest (will load)',
      );
      lines.push(
        '- ⚠️  **Orphaned**: File exists but NOT in manifest (will NOT load)',
      );
      lines.push(
        '- ❌ **Missing**: In manifest but file missing (will fail to load)',
      );
    }
    ...
    lines.push(
      '- To initialize a writable BMAD directory: `bmad *init --help`',
    );
    if (
      stats.orphanedAgents + stats.orphanedWorkflows + stats.orphanedTasks >
      0
      ...
      lines.push(
        '- To add orphaned files to manifest: Update CSV files in `_cfg/`',
      );
    }
