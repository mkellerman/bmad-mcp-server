...
async function loadSessionMetadata() {
  const metadataPath = path.join('test-results', 'session-metadata.jsonl');
...
async function generateReport() {
  ...
  // Load session metadata (test names)
  ...
  for (const file of sessionFiles) {
    ...
    if (events.length > 0) {
      ...
      // Add test name from metadata if available
      const metadata = sessionMetadata.get(analysis.sessionId);
      if (metadata) {
        analysis.testName = metadata.testName;
      }
  ...
  for (const session of sessions) {
    const row = [
      ...
      escapeCSV(session.testName || 'N/A'),
  ...
  console.log(
    `Avg Tool Calls:     ${(totalToolCalls / sessions.length).toFixed(1)}`,
  );
  console.log(
    `Total Tokens:       ${totalTokens > 0 ? totalTokens.toLocaleString() : 'N/A'}`,
  );
  console.log(
    `Avg Tokens/Session: ${totalTokens > 0 ? Math.round(totalTokens / sessions.length).toLocaleString() : 'N/A'}`,
  );
  ...
  sessions.slice(0, 5).forEach((session, i) => {
    ...
    console.log(
      `   Tools:   ${session.toolCallCount} calls (${session.toolNames || 'none'})`,
    );
